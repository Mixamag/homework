...
enum TextErrors: String {
    case CARD_NUMBER_OR_PIN_INCORRECT = "Данные карты введены некорректно"
...
enum UserActions {
    ...
    case userPressedTopUpAccount(topUp: Float)   //пополнить деньги на депозит
...
struct BankServer: BankApi {
    ...
    func showWithdrawalDeposit(cash: Float) {
        ...
        У вас останоль наличных: \(user.userCash) рублей.
        Хорошего дня!
        """
        print(report)
    }
    func showTopUpAccount(cash: Float) {
        ...
        Сумма депозита состаяляет: \(user.userBankDeposit) рублей.
        У вас останоль наличных: \(user.userCash) рублей.
        Хорошего дня!
        """
        print(report)
...
// Пользовательские данные на сервере банка (внесены при регистрации)
let egor_pupkin: UserData = User(
    userName: "Egor Pupkin",
    userCardId: "1111 2222 3333 4444 5555",
    userCardPin: 1234,
    userCash: 2222.22,
    userBankDeposit: 5555.55,
    userPhone: "8-555-555-55-55",
    userPhoneBalance: -34.33
)

//банк, где хранятся данные пользователя
let bankClient = BankServer(user: egor_pupkin)

// текущий банк, где осуществляется работа на данный момент

let atm443 = ATM(
    userCardId: "1111 2222 3333 4444 5555",
    userCardPin: 1234,
    someBank: bankClient,
    action: .userPressedPayPhone(phone: "8-555-555-55-55"),
